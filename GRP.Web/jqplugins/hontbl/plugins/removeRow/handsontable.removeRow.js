        ...
        var newRowHeader = function (row, elem) {
          ...
          if (row > -1) {
              ...
              eventManager.addEventListener(div, 'mouseup', function () {
                  if (estHO.getDataAtRowProp(row, 'eCode') != null) {
                      $.messager.confirm('تنبيه', 'سيتم حذف السجل:<br/>رقم الموظف: ' + estHO.getDataAtRowProp(row, 'eCode') + '<br/>اسم الموظف: ' + estHO.getDataAtRowProp(row, 'eName'), function (r) {
                          if (r) {
                              deleteFromOprMTList(row);
